...
export function pushNotification(n: Omit<AppNotification, "id" | "read" | "at">) {
  ...
  if (supa) {
    void supa.from("notifications").insert({
      kind: n.kind,
...
export function useNotifications(email?: string) {
  ...
  const list = email ? all.filter((n) => n.targetEmail.toLowerCase() === email.toLowerCase()) : all;
  ...
  const markAllRead = useCallback(() => {
    const supa = getSupabase();
    setAll((prev) =>
      prev.map((n) => (email && n.targetEmail.toLowerCase() !== email.toLowerCase() ? n : { ...n, read: true })),
    );
    if (supa) {
      ...
      writeLS(
        all.map((n) => (email && n.targetEmail.toLowerCase() !== email.toLowerCase() ? n : { ...n, read: true })),
      );
    }
  }, [email, all]);
  ...
  const clear = useCallback(() => {
    const supa = getSupabase();
    setAll((prev) => (email ? prev.filter((n) => n.targetEmail.toLowerCase() !== email.toLowerCase()) : []));
    if (supa) {
